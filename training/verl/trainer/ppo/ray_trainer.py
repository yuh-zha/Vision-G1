...
from verl.trainer.ppo.metric_utils import compute_data_metrics, compute_throughout_metrics, compute_timing_metrics, reduce_metrics, compute_difficulty_histogram_metrics, \
                                            bootstrap_metric, calc_maj_val, process_validation_metrics, sample_outputs
...
class RayPPOTrainer(object):
    ...
    def _validate(self):
        ...
        def _save_inputs_outputs_scores(inputs, outputs, scores, data_sources):
            import pandas as pd
            ...
            df.to_parquet(
                os.path.join(self.config.trainer.default_local_dir, f'global_step_{self.global_steps}', 'val_results.parquet'),
                index=False,
                engine='pyarrow',
            )
        
        if self.global_steps % self.config.trainer.save_freq == 0:
            _save_inputs_outputs_scores(sample_inputs, sample_outputs, sample_scores, data_sources)
